...
namespace Arch
{
	public class NamedWorld : Singleton<NamedWorld>
	{
		...
		public static World DefaultWord
		{
			get
			{
				if (Instance.m_defaultWorld == null)
				{
					if (!m_dicWorlds.TryGetValue("Default", out Instance.m_defaultWorld))
					{
						m_dicWorlds.Add("Default", World.Create());
						Instance.m_defaultWorld = m_dicWorlds["Default"];
					}
				}
				return Instance.m_defaultWorld;
			}
		}
