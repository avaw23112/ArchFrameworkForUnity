...
namespace Arch.Net
{
	...
	public static class StructCommandQueue
	{
		...
		private static readonly Dictionary<Type, Action<Entity>> s_addCache = new();
		private static readonly Dictionary<Type, Action<Entity>> s_removeCache = new();
		...
		private static void ApplySingle(byte[] payload)
		{
			...
			if (first == (byte)RpcIds.StructCommandGroup)
				p++;
			...
			if (!NamedWorld.TryGetById(worldId, out var world))
				world = NamedWorld.DefaultWord;
		...
		{
			if (!s_addCache.TryGetValue(compType, out var action))
			{
				var mi = typeof(StructCommandQueue)
					.GetMethod(nameof(AddGeneric), System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic)!
					.MakeGenericMethod(compType);

				action = (Action<Entity>)mi.CreateDelegate(typeof(Action<Entity>));
				s_addCache[compType] = action;
			}
			action(entity);
		}
		...
		{
			if (!s_removeCache.TryGetValue(compType, out var action))
			{
				var mi = typeof(StructCommandQueue)
					.GetMethod(nameof(RemoveGeneric), System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic)!
					.MakeGenericMethod(compType);

				action = (Action<Entity>)mi.CreateDelegate(typeof(Action<Entity>));
				s_removeCache[compType] = action;
			}
			action(entity);
		}

		private static void AddGeneric<T>(Entity e) where T : struct, IComponent
		{
			if (e.IsAlive() && !e.Has<T>())
				e.Add<T>();
		}

		private static void RemoveGeneric<T>(Entity e) where T : struct, IComponent
		{
			if (e.IsAlive() && e.Has<T>())
				e.Remove<T>();
		}
