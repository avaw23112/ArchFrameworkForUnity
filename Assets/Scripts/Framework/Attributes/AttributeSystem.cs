...
namespace Attributes
{
    ...
    public abstract class MutipleAttributeSystem<T> : IAttributeProcessor where T : BaseAttribute
    {
        public void Process()
        {
            ...
            foreach (var kvp in attributes)
            {
                ...
                var list = kvp.Value;
                Process(derectType, list);
            }
            DictionaryPool<Type, List<T>>.Release(attributes);
        }

        public abstract void Process(Type derectType, List<T> list_T);
    }
        ...
        where T2 : BaseAttribute
    {
        ...
        public abstract void Process(Type derectType, List<T1> list_T1, List<T2> list_T2);
    }
        ...
        where T3 : BaseAttribute
    {
        public void Process()
        {
            ...
            foreach (var kvp in attributes)
            {
                ...
                Process(derectType, list_T1, list_T2, list_T3);
            }
            DictionaryPool<Type, (List<T1>, List<T2>, List<T3>)>.Release(attributes);
        }

        public abstract void Process(Type derectType, List<T1> list_T1, List<T2> list_T2, List<T3> list_T3);
    }
    ...
    public abstract class AttributeSystem<T> : IAttributeProcessor where T : BaseAttribute
    {
        ...
        public abstract void Process(T attribute, Type derectType);
    }
        ...
        where T2 : BaseAttribute
    {
        public void Process()
        {
            ...
            foreach ((T1 attribute, T2 attribute2, Type derectType) in attributes)
            {
                Process(attribute, attribute2, derectType);
            }
            ListPool<(T1, T2, Type)>.Release(attributes);
        }

        public abstract void Process(T1 attribute_T1, T2 attribute_T2, Type derectType);
    }
