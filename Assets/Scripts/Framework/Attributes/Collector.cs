...
namespace Attributes
{
    ...
    internal static class Collector
    {
        ...
        /// 检查标记类上是否带有Forget属性
        /// </summary>
        private static bool isForget(Type derectType)
        {
            if (derectType == null)
            {
                throw new NullReferenceException("derectType is null");
            }
            if (derectType == typeof(EntitasSystemTest_2_System))
            {
                ;
            }
        ...
        /// <summary>
        /// 检查标记类上是否带有其他Collectable属性
        /// </summary>
        private static bool isCollectable<T>(Type derectType)
            where T : Attribute
        {
            if (derectType == null)
            {
        ...
        public static void CollectAttributes<T>(List<(T, Type)> listAttributes) where T : Attribute
        {
            ...
            if (Attributes.TryGetDecrectType(typeof(T), out dictAttributes))
            {
                ...
                foreach (var item in dictAttributes)
                {
                    ...
                    {
                        Tools.Logger.Debug("请用MultiCollectAttributes方法收集多重属性");
                        continue;
                    }
            ...
            where T2 : Attribute
        {
            ...
            //如果在收集之前就已经被Forget掉，就会导致取空，直接返回
