...
namespace Arch.Net
{
	...
	public static class StructCommandAggregator
	{
		...
		private static void SubscribeEntityEvents(World w)
		{
			...
			w.SubscribeComponentAdded((in Entity e, ref NetworkEntityId nid) =>
			{
				if (e.TryGet<NetworkOwner>(out var owner) && OwnershipService.IsOwner(owner.OwnerClientId))
		...
		private static int SumVarInt(HashSet<int> set)
		{ int s = 0; foreach (var v in set) s += VarIntSize((uint)v); return s; }

		private static void WriteVarUInt(byte[] buf, ref int p, uint v)
		{ while (v >= 0x80) { buf[p++] = (byte)(v | 0x80); v >>= 7; } buf[p++] = (byte)v; }
