...
namespace Arch.Net
{
    ...
    public static class ManifestSerializer
    {
        ...
        public static byte[] BuildArchetypeManifestV2(World world)
        {
            ...
            // Prepare buffer
            var archIds = new System.Collections.Generic.List<uint>();
            ...
            while (true)
            {
                ...
                {
                    if (tmp[i] == 0) continue; // assume 0 is not a valid archId
                    archIds.Add(tmp[i]);
                }
                break;
            }

            var entries = new System.Collections.Generic.List<ArchetypeEntryV2>(archIds.Count);
            foreach (var archId in archIds)
            {
                ...
                var pairs = new System.Collections.Generic.List<(int idx, int typeId)>();
                ...
                for (int i = 0; i < pairs.Count; i++) typeIds[i] = pairs[i].typeId;
        ...
        /// </summary>
        public static System.Collections.Generic.List<ArchetypeEntryV2> ParseArchetypeManifestV2(byte[] payload, int offset, int length)
        {
            var res = new System.Collections.Generic.List<ArchetypeEntryV2>();
            int p = offset;
            int end = offset + length;
            if (p >= end) return res;
