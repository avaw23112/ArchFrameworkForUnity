...
namespace Arch.Net
{
    ...
    public sealed partial class SyncScanSystem : GlobalUpdateSystem<NetworkRuntime>
    {
        ...
        internal struct OwnedBatch
        {
            ...
            {
                if (!Pooled) return;
                try { if (Ids != null) System.Buffers.ArrayPool<byte>.Shared.Return(Ids); } catch { }
                try { if (Values != null) System.Buffers.ArrayPool<byte>.Shared.Return(Values); } catch { }
                Ids = null; Values = null; Pooled = false;
            }
        }

        internal static class SyncScanSystemHelpers
        {
            public static unsafe void WriteUnaligned<T>(byte[] dst, int offset, T value) where T : struct
            {
        ...
        private static OwnedBatch CollectOwned<T>(World world, int maxEntities) where T : struct, IComponent
        {
            ...
            if (n <= 0)
            {
                return new OwnedBatch { Ids = null, IdsLength = 0, Values = null, ValuesLength = 0, Count = 0, ElemSize = elemSize, Pooled = false };
            }
            ...
            world.Query(in q, (Entity e, ref T c, ref NetworkEntityId nid, ref NetworkOwner owner) =>
            {
                ...
                SyncScanSystemHelpers.WriteUnaligned(valBytes, i * elemSize, c);
                i++;
            });
            return new OwnedBatch { Ids = idsBytes, IdsLength = idsLen, Values = valBytes, ValuesLength = valsLen, Count = n, ElemSize = elemSize, Pooled = true };
        }
